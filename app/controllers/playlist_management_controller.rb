	...
	private
		...
		def update_mpd(channel_playlist)
			...
			playlist_entries.each do |entry|
				...
				mpd.add "file://" + File.join(Rails.root, 'public', entry.episode.audio.path).path if entry.is_episode?
				mpd.add "file://" + File.join(Rails.root, 'public', entry.jingle.audio.path).path if entry.is_jingle?
